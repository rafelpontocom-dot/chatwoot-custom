...
const referencia = t => `<!doctype html>
<html lang="pt"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Raevo — referência do sistema</title>
<style>
  :root { color-scheme: light; --bg:#F7F8FA; --card:#fff; --ink:#111827;
          --muted:#7A828F; --line:#E9EBEF; }
  [data-tema="dark"] { color-scheme: dark; --bg:#0F1115; --card:#1A1F26;
          --ink:#E8EAEE; --muted:#858D9A; --line:#262B33; }
  * { box-sizing: border-box; }
  ...
  button { font:inherit; border:1px solid var(--line); background:var(--card);
           color:var(--ink); border-radius:9999px; padding:6px 14px; cursor:pointer; }
...
<header>
  ...
  <span class="meta">aprovado ${esc(t.approved)} · gerado de <code>${esc(P.tokens)}</code></span>
  <button id="tema" type="button">Alternar tema</button>
</header>
<main>
  <p class="aviso">Esta página é <strong>gerada</strong> dos tokens. Se um valor aqui
  estiver errado, o erro está no sistema — não nesta página. Para mudar, edite
  <code>${esc(P.tokens)}</code> e o arquivo de origem, e rode
  <code>pnpm raevo:tokens</code>.</p>
  ...
  ${['micro:11px', 'xs:12px', 'sm:14px', 'base:16px', 'xl:20px', '3xl:30px']
    .map(par => {
      const [n, px] = par.split(':');
      return `<tr><td><code>text-${n}</code> · ${px}</td>
        <td style="font-size:${px}">Agendamento confirmado</td></tr>`;
    })
    .join('')}
  </table>
...
if (comando === 'extract') {
  ...
  gravar(P.reference, referencia(t));
  console.log(`✓ ${P.reference} gerado de ${P.tokens}.`);
  ...
  try {
    referenciaVelha = ler(P.reference) !== referencia(JSON.parse(ler(P.tokens)));
  } catch {
  ...
  if (falhas.length || deriva.length || referenciaVelha) {
    console.error(
  ...
  console.log(`✓ ${cores} tokens de cor, ${codigo.stage.palette.length + 1} etapas e a escala de forma em acordo.`);
} else {
  console.error('uso: node scripts/design-tokens.mjs [extract|check|reference]');
  process.exit(1);
}
