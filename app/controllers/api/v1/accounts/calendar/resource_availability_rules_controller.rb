class Api::V1::Accounts::Calendar::ResourceAvailabilityRulesController < Api::V1::Accounts::BaseController
  ...
  def index
    render json: @resource.kanban_calendar_availability_rules.order(:kind, :weekday, :starts_at_local).map { |rule| rule_payload(rule) }
  end

  def create
    rule = @resource.kanban_calendar_availability_rules.create!(rule_params)
  ...
  def fetch_resource
    @resource = policy_scope(KanbanCalendarResource).find(params[:resource_id])
  end

  def fetch_rule
    @rule = @resource.kanban_calendar_availability_rules.find(params[:id])
  end
