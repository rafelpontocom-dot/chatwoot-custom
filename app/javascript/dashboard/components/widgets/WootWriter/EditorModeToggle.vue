...
<template>
  <button
    ...
    :aria-label="
      isPrivate
        ? $t('CONVERSATION.REPLYBOX.MODE_INTERNAL_ACTIVE')
        : $t('CONVERSATION.REPLYBOX.MODE_REPLY_ACTIVE')
    "
    :disabled="disabled || isReplyRestricted"
    :class="[
      isPrivate ? 'border-n-amber-9' : 'border-transparent',
  ...
  >
    ...
    <!--
      O cadeado vem com o nome: «interna» diz de quem é a mensagem, e o ícone
      diz o mesmo a quem lê o ecrã pela forma e não pela cor.
    -->
    <div
      ref="wootEditorPrivateMode"
      data-testid="editor-mode-internal"
      class="flex items-center gap-1 px-2 z-20"
      :class="isPrivate ? 'text-n-amber-12' : 'text-n-slate-11'"
    >
      <i class="i-lucide-lock size-3.5" aria-hidden="true" />
      {{ $t('CONVERSATION.REPLYBOX.INTERNAL_NOTE') }}
    </div>
    <div
