...
`custom_field_sections` representa uma superfície de trabalho, e não um agrupamento técnico obrigatório. Uma nova seção deve existir somente quando possui uma rotina própria. Dentro da seção, `layout.group` identifica uma pergunta comercial ou decisão, e os campos são renderizados como linhas de leitura compactas com edição contextual. Financeiro, por exemplo, pode usar os grupos `quanto_foi_combinado`, `como_sera_pago`, `pagamento_aconteceu` e `informacoes_internas`; forma de pagamento e data de pagamento não exigem uma aba individual.

O card apresenta `Geral`, `Contato`, `Agente`, `Agenda` quando habilitada, `Marketing`, as abas personalizadas e `Linha do tempo`. O título da oportunidade aparece no cabeçalho e entra em edição somente pelo botão de lápis. O mesmo cabeçalho apresenta identificador, board e seletor de etapa; uma mudança para categoria `won` ou `lost` é persistida pelo update normal do card, sendo que `lost` requer `lost_reason`. A conversa vinculada é aberta por ação de ícone no cabeçalho. `Contato` renderiza telefone, e-mail, identificador, atributos e etiquetas da conta como lista vertical com quebra segura de texto; `Agente` somente apresenta a atribuição derivada da conversa. A área de Agenda só existe como aba própria. Inputs de edição usam placeholder ou opção inicial e `aria-label`; controles nativos de data preservam `aria-label` pois o formato visual é fornecido pelo navegador. Datas `starts_at` e `due_at` pertencem ao planejamento interno e ficam recolhidas, para não serem confundidas com reservas da Agenda. Para administradores, uma engrenagem abre o gerenciador do board e o botão `+` inicia a criação de uma aba sem exigir saída manual do contexto da oportunidade.
...
O cabeçalho operacional é dividido em duas faixas: a primeira contém seletor do board, busca unificada, criação rápida e menu de ações; a segunda contém alternância Kanban/Lista e abertura da Central `Hoje`. Clicar ou focar a busca abre o painel de filtros contextual, com `aria-expanded`, `aria-controls`, suporte a `Escape` e sem uma segunda linha permanente de controles. A busca aceita `Enter`, possui ação visível de aplicar e limpeza independente, que preserva os demais filtros; ela consulta assunto da oportunidade, nome do contato, telefone e e-mail. Ordenação e filtros salvos são renderizados somente dentro desse painel. Exportação, resumo comercial, automações e configurações ficam no menu de ações para preservar a largura do canvas. Na visão em lista, a seleção do cabeçalho atua exclusivamente sobre cards visíveis e já carregados; cards fora da página não entram na operação em massa por inferência.
