...
O layout desktop do editor tem três camadas: paleta lateral operacional fixa em `12.25rem` (196 px), canvas flexível com altura mínima de `42rem` e inspector contextual sobreposto de até `16rem` (256 px). A paleta nunca cresce com o conteúdo dos blocos; o cabeçalho exibe busca e contagem total, as categorias são recolhíveis, apresentam sua própria contagem, preservam a escolha de abertura após nova renderização e seus itens têm ícone em bloco, título truncável e área de clique/arraste estável. O inspector é `fixed`, fica à direita, começa abaixo do cabeçalho do produto e tem altura limitada ao conteúdo/viewport; ele abre como folha contextual, nunca como coluna permanente nem drawer alto em desktop. Em telas pequenas, ele ocupa a viewport com margens seguras.
