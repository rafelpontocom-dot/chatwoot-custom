...
- Editor visual com estrutura, prévia interativa e propriedades essenciais: a secretaria monta o formulário vendo a experiência do paciente, sem precisar navegar por chaves, destinos CRM ou regras técnicas. Publicação, integrações, condicionais e mapeamentos ficam em uma área avançada recolhida.
- A prévia permite testar condições com respostas temporárias, alternar entre desktop e celular e limpar a simulação sem tocar no schema. Etapas e perguntas podem ser reordenadas visualmente; controles de mover acima/abaixo continuam como alternativa de teclado.
