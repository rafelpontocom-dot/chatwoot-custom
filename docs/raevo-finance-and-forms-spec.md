...
O editor disponibiliza blocos comerciais locais e editáveis para inserir dados de contato, preferência de agenda e origem/interesse. Cada inserção cria seção e campos próprios no schema da versão, resolve chaves duplicadas com sufixo estável e não introduz informações clínicas. Seções aceitam `description` textual curta; a descrição vazia não é publicada. A ordem de seções e perguntas pode ser alterada por botões acessíveis de mover acima/abaixo, preservando chave, descrição e configurações do item. A biblioteca por conta, compartilhamento de grupos entre modelos, reordenação por arrastar e grupos de saúde dependem das permissões e retenção do P1 clínico.
...
`FormAccessAudit` registra `account_id`, `form_submission_id`, `actor_id`, ação `view` e horário, sem copiar conteúdo clínico. O P1 clínico entrega armazenamento cifrado separado, convite individual de uso único, consentimento e auditoria de leitura administrativa. Índice não sensível, snapshot de consentimento, matriz por profissional/unidade, retenção, exportação e anexos dependem da próxima etapa clínica.
...
- `Forms::SubmissionPolicy`: nesta primeira entrega clínica, somente administrador da conta lê o conteúdo detalhado de anamnese; evoluir para matriz por unidade/profissional antes de liberar a clínica em escala.
...
- Arquivos P1: allowlist MIME/extensão, limite de tamanho, nome interno aleatório, armazenamento privado, URL assinada, varredura e autorização por download.
