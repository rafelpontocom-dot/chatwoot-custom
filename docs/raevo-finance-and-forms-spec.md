...
`schema.logics` é uma lista por pergunta: `{ field_key, payloads: [{ id, condition, action }] }`. `condition` é `{ ref, comparison, expected }`, onde `ref` é a chave de uma pergunta publicada anterior. `action` é `{ kind, field_key }` para `navigate` ou `{ kind, variable, operator, value }` para `calculate`. As regras de uma pergunta são avaliadas em ordem e a primeira que se cumpre é a que vale; nenhuma outra corre para aquele respondente.

`comparison` aceita, por tipo de pergunta:

| Tipo | Operadores |
| --- | --- |
| texto curto/longo, e-mail, telefone | `is`, `is_not`, `contains`, `does_not_contain`, `starts_with`, `ends_with` |
| seleção simples, consentimento | `is`, `is_not` |
| seleção múltipla | `is`, `is_not`, `contains`, `does_not_contain` |
| número, moeda | `equal`, `not_equal`, `greater_than`, `greater_or_equal_than`, `less_or_equal_than` |
| data, data-hora | `is`, `is_not`, `is_before`, `is_after` |
| qualquer tipo | `is_empty`, `is_not_empty` |

`calculate.operator` aceita `addition`, `subtraction`, `multiplication`, `division` e `assignment`. Variáveis vivem em `schema.variables` como `{ id, name, kind, initial }`, nunca são apresentadas ao respondente e nunca entram no mapeamento CRM. Campos ocultos vivem em `schema.hidden_fields`; num convite individual eles são preenchidos pelo servidor a partir do token e jamais pela URL. Condições combinam-se com `all` ou `any` num único nível — parênteses não são aceites, para a regra continuar legível.

`Forms::SchemaValidator` recusa publicar quando uma regra referencia chave inexistente, quando o operador não pertence ao tipo da pergunta, quando `navigate` aponta para pergunta anterior a si própria criando ciclo, ou quando uma variável referenciada não existe. Para `sensitive_health`, `calculate` é permitido e `navigate` também, mas nenhuma variável pode ser mapeada para CRM.

No envio, o servidor reavalia a mesma lógica que a interface usou e só persiste respostas de perguntas efetivamente apresentadas. Campo `hidden`, resposta de uma condicional que não foi apresentada e chave enviada fora do schema nunca entram em `FormSubmission`, no mapeamento de contato ou no destino comercial.

O editor é uma superfície visual de três áreas persistentes, nenhuma em diálogo: estrutura e biblioteca pesquisável à esquerda; ao centro o formulário real, que é simultaneamente prévia e superfície de edição; à direita um painel contextual com as abas `Pergunta`, `Design` e `Lógica`, que segue a seleção. O editor é rota de tela inteira e recolhe a barra lateral do CRM. Configuração de campo, aparência e lógica nunca abrem sobre o canvas. A página central nunca grava respostas; `Abrir prévia` abre o renderer público real em uma nova aba privada. Criar pergunta ou inserir bloco mantém a seleção no item novo. Identificadores, mapeamentos CRM, condicionais, publicação e destino ficam recolhidos em `Configurações avançadas`, preservando o fluxo diário de uma secretaria sem ocultar controles de configuração.

#### Onde a resposta vive

A resposta pertence ao contacto; o envio pertence à oportunidade. `FormSubmission` continua a referenciar `contact_id` e `kanban_card_id`, mas com papéis distintos: o conteúdo é histórico da pessoa e sobrevive ao ciclo do card; o envio é um evento datado, preso ao atendimento que o originou e à `FormTemplateVersion` que o recebeu. A ficha do contacto lista a série completa em ordem decrescente; a oportunidade lista apenas os envios daquele card.

Nenhum envio sobrescreve outro. Para `sensitive_health`, o contacto guarda a série e nunca o estado atual: uma resposta clínica não vira `custom_attribute`, porque um atributo sugere estar em dia quando pode ter meses. `Forms::MapSubmissionToCrmService` e `Forms::MapSubmissionToOpportunityService` mantêm a recusa já existente para `access_classification == 'sensitive_health'`. Formulários de cadastro e de dados fiscais são o caso oposto e continuam a atualizar o contacto pelo mapeamento explícito.
