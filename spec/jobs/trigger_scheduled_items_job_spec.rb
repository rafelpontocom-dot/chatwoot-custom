...
RSpec.describe TriggerScheduledItemsJob do
  ...
  it 'triggers KanbanAutomations::DispatchOverdueNextActionsJob' do
    expect(KanbanAutomations::DispatchOverdueNextActionsJob).to receive(:perform_later).once
    described_class.perform_now
  end
  ...
  it 'triggers KanbanCalendar::ImportAllFeegowCalendarsJob' do
    expect(KanbanCalendar::ImportAllFeegowCalendarsJob).to receive(:perform_later).once
    described_class.perform_now
  end
  ...
  it 'triggers Forms::ExpireInvitationsJob' do
    expect(Forms::ExpireInvitationsJob).to receive(:perform_later).once
    described_class.perform_now
  end

  it 'triggers Forms::DetectAbandonedInvitationsJob' do
    expect(Forms::DetectAbandonedInvitationsJob).to receive(:perform_later).once
    described_class.perform_now
  end
